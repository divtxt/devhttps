...
func GenerateCaddyfile(cfg *config.Config) (string, error) {
	...
	buf.WriteString(fmt.Sprintf("# Generated by devhttps at %s\n\n", time.Now().Format(time.RFC3339)))
	for _, entry := range cfg.Entries {
		fullchainPath := fmt.Sprintf("{$HOME}/.devhttps/certbot/config/live/%s/fullchain.pem", entry.Domain)
		privkeyPath := fmt.Sprintf("{$HOME}/.devhttps/certbot/config/live/%s/privkey.pem", entry.Domain)
